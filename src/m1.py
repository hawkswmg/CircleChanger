...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        radius = other_circle_changer.circle.center.get_distance_from(
            self.circle.center) / 2
        center = other_circle_changer.circle.center.halfway_to(
            self.circle.center)
        new_circle = CircleChanger(center.x, center.y, radius, 'red',
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.color_change_index = self.color_change_index + 1
        if self.color_change_index == len(self.colors):
            self.color_change_index = 0

        self.circle.fill_color = self.colors[self.color_change_index]
